...
Template.registerHelper(
  'hasManagerCreatedVault',
  () => Vaults.find({ owner: Session.get('selectedAccount') }).count() !== 0,
);
Template.registerHelper(
  'getManagerAddressOfVault',
  () =>
    Vaults.find({ owner: Session.get('selectedAccount') }).count() !== 0
      ? Vaults.findOne({ owner: Session.get('selectedAccount') }).address
      : false,
);
Template.registerHelper(
  'getManagerNameOfVault',
  () =>
    Vaults.find({ owner: Session.get('selectedAccount') }).count() !== 0
      ? Vaults.findOne({ owner: Session.get('selectedAccount') }).name
      : false,
);
Template.registerHelper(
  'getManagerDeltaOfVault',
  () =>
    Vaults.find({ owner: Session.get('selectedAccount') }).count() !== 0
      ? convertFromTokenPrecision(
          Vaults.findOne({ owner: Session.get('selectedAccount') }).delta,
          18,
        )
      : false,
);
Template.registerHelper(
  ...
  () =>
    Vaults.find({ owner: Session.get('selectedAccount') }).count() !== 0
      ? Vaults.findOne({ owner: Session.get('selectedAccount') }).sharePrice
      : false,
);
Template.registerHelper(
  'getIsNewOfVault',
  () =>
    Vaults.find({ owner: Session.get('selectedAccount') }).count() !== 0
      ? !Vaults.findOne({ owner: Session.get('selectedAccount') }).isUsed
      : false,
);
Template.registerHelper(
  'getIsFundedVault',
